...
ComputeDerivatives::ComputeDerivatives(LAMMPS *lmp, int narg, char **arg) :
  ...
  cutsq(0.0), rs(NULL), eta_G2(NULL), zeta(NULL), eta_G4(NULL), lambda(NULL), alocal(NULL)

{
  ...
  int n_derivatives = n_etaG2*ntypes*g2_flag*n_rs + n_lambda*n_zeta*n_etaG4*ntypes_combinations*g4_flag + ntypes;
...
void ComputeDerivatives::compute_derivatives()
{
  ...
  double Rx_ij, Ry_ij, Rz_ij, rsq, rsq_g2, Rij, Rx_ik, Ry_ik, Rz_ik, rsq1;
    ...
    if (mask[i] & groupbit) {
      ...
      for (int jj = 0; jj < jnum; jj++) {            // Loop for the first neighbor j
        ...
        if (cutsq-rsq>nb_pad && rsq>1e-20) { 
          function = 0.5*(cos(Rij*pi/cut)+1);
          dfc = -0.5*sin(pi*Rij/cut)*pi/cut;
          ...
          // compute G2
          if (g2_flag == 1) {
            // The number of G2 derivatives depend on the number of given eta_G2 parameters and rs parameters
            for (int m = 0; m < n_etaG2; m++)  {
              for (int n = 0; n < n_rs; ++n){
                rsq_g2 = pow((Rij-rs[n]), 2);
                int idx = ntypes + m*n_rs + n + (jtype-1)*n_etaG2*n_rs + 3;
                
                // cout << "    idx:" << idx << "  m:" << m << "  eta_G2[m]:" << eta_G2[m] << "  n:" << n << "  rs[n]:" << rs[n]<< "  jtype:" << jtype << endl;

                derivatives[0][idx] = exp(-eta_G2[m]*rsq_g2) * (dfc - 2*eta_G2[m]*(Rij-rs[n])*function)*Rx_ij/Rij; // G2 derivatives in the x direction
                derivatives[1][idx] = exp(-eta_G2[m]*rsq_g2) * (dfc - 2*eta_G2[m]*(Rij-rs[n])*function)*Ry_ij/Rij; // G2 derivatives in the y direction
                derivatives[2][idx] = exp(-eta_G2[m]*rsq_g2) * (dfc - 2*eta_G2[m]*(Rij-rs[n])*function)*Rz_ij/Rij; // G2 derivatives in the z direction

                derivatives_i[0][idx] += -exp(-eta_G2[m]*rsq_g2) * (dfc - 2*eta_G2[m]*(Rij-rs[n])*function)*Rx_ij/Rij; // G2 derivatives in the x direction
                derivatives_i[1][idx] += -exp(-eta_G2[m]*rsq_g2) * (dfc - 2*eta_G2[m]*(Rij-rs[n])*function)*Ry_ij/Rij; // G2 derivatives in the y direction
                derivatives_i[2][idx] += -exp(-eta_G2[m]*rsq_g2) * (dfc - 2*eta_G2[m]*(Rij-rs[n])*function)*Rz_ij/Rij; // G2 derivatives in the z direction 
              }
            }
          }
          ...
          if (g4_flag == 1) {
            for (int kk = 0; kk < jnum; kk++) {            // Loop for the second neighbor k
              ...
              if (cutsq-rsq1>nb_pad && rsq1>1e-20 && cutsq-rsq2>nb_pad && rsq2>1e-20) {
                  ...
                  if (aux > 0)  {
                    for (int l = 0; l < n_zeta; l++)  {
                      for (int q = 0; q < n_etaG4; q++) {
                        ...
                        int idx_g4 = ntypes + n_etaG2*n_rs*ntypes + h + n_lambda*(l+n_zeta*(q+n_etaG4*type_comb)) + 3;

                        // G4 derivatives calculation
                        if ( kk != jj ) {
                          // G4 derivatives with respect to x, y, z directions
                          derivatives[0][idx_g4]  += G4*(ij_factor*Rx_ij-jk_factor*Rx_jk);
                          derivatives[1][idx_g4]  += G4*(ij_factor*Ry_ij-jk_factor*Ry_jk);
                          derivatives[2][idx_g4]  += G4*(ij_factor*Rz_ij-jk_factor*Rz_jk);
                        }
                        if ( kk > jj ) {
                          derivatives_i[0][idx_g4]  += -G4*(ij_factor*Rx_ij+ik_factor*Rx_ik);
                          derivatives_i[1][idx_g4]  += -G4*(ij_factor*Ry_ij+ik_factor*Ry_ik);
                          derivatives_i[2][idx_g4]  += -G4*(ij_factor*Rz_ij+ik_factor*Rz_ik);
                        }
                      }
                    }
                  }
